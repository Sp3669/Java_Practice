...
public class StudentGradeDataAssignment {
	...
	public static void main(String[] args) {
		 ...
		 * Part 3 — Logic
		 * 
		 * Print each student with all their remarks 
		 * Print only students with score above 80 
		 * Print only students who have remark "Excellent"
		 * 
		 * Part 4
		 * 
		...
		System.out.printf("|name: %s |age: %d|course: %s|score: %.2f|remarks: %s , %s%n",
				          Students.get(i).name,
				          Students.get(i).age,
				          Students.get(i).course,
				          Students.get(i).score,
				          Students.get(i).remarks.get(0),
		...
		for(int j =0; j< Students.size(); j++) {
		    if (Students.get(j).score == lowestScore) {
		    	System.out.printf("remarks: %s , %s%n",
		    			Students.get(j).remarks.get(0),
		    			Students.get(j).remarks.get(1));
		...
		//print
				for(int i =0; i< Students.size(); i++) {
					if(Students.get(i).remarks.isEmpty()) {
					    System.out.printf("|name: %s |score: %.2f|remarks: EMPTY%n",
					        Students.get(i).name, Students.get(i).score);
					} else {
					    System.out.printf("|name: %s |remarks: %s , %s%n",
					        Students.get(i).name,
					        Students.get(i).remarks.get(0),
					        Students.get(i).remarks.get(1));
					}
